...
export const GET = async () => {
    try {
        ...
        if (session.user.role === "Admin" || session.user.role === "Owner") {
            ...
            const [productsCount, users, totalSoldResult, lowStockProducts, topSellingStaffResult] = await Promise.all([
            ...
            return NextResponse.json({ productsCount, users, totalSold, topSellingStaff: topSellingStaffResult, lowStockProducts })
        } else {
            // Convert userId to ObjectId for aggregate query
            const userObjectId = new mongoose.Types.ObjectId(session.user.id);

            const [productsCount, totalSoldResult, lowStockProducts] = await Promise.all([
